...
do.chunk.of.names <- function(names, country.code = NULL, names.already.done = NULL) {
  ...
  if (is.null(country.code)) {
    search.result <- call.api(paste0(c(base.url, names), collapse = "&name[]="))
  } else {
    search.result <- call.api(paste(paste0(c(base.url, names), collapse = "&name[]="), "&country_id=", country.code, sep = ""))
  }
  ...
  if ("gender" %in% names(search.result)) {
    if (!is.null(search.result$gender)) output$gender <- search.result$gender
  } # NOT THIS
  ...
  if (is.null(names.already.done)) {
    return(output)
  } else {
    (return(list(output, names.already.done)))
  } # return the name hits and the up-to-date version of names.already.done
}
...
do.many.names <- function(names, country.code = NULL, names.already.done = NULL) {
  ...
  # Or if we entered 10 or fewer names...
    ...
    output <- do.chunk.of.names(names = names, country.code = country.code)
  }
...
do.many.names.in.blocks <- function(names, output.dir, country.code = "unspec", names.already.done = NULL, chunk.size = 1000) {
  ...
  for (i in 1:length(remaining.to.do)) {
    ...
    if (country.code != "unspec") {
      ...
      chunk.o.names <- do.many.names(focal.names)
    }
...
unique.names.by.country <- lapply(acceptable.countries, function(focal.country) {
  print(focal.country)
  focal <- (pubmed_sqlite %>% filter(country == focal.country) %>% select(forenames) %>% collect(n = Inf) %>% as.data.frame())[, 1] %>%
    strsplit(split = "_") %>%
    unlist() %>%
    unique()
  focal <- focal %>%
...
x <- read.csv("../data/country codes.csv", stringsAsFactors = F) %>%
  filter(code %in% code.list) %>%
...
names.already.done <- names.already.done %>%
  filter(country == "unspec") %>%
  select(name)
