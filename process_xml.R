...
averageIF <- function(journal.family, IFdata) {
  ...
  IFdata <- rbind(IFdata, rep(NA, ncol(IFdata)))
  IFdata[nrow(IFdata), 2] <- journal.family
...
ends <- seq(chunk.size, nChunks * chunk.size, by = chunk.size)
ends[length(ends)] <- nrow(pubmed)
...
missing.ones %>%
  filter(is.na(broad.journal.headings)) %>%
  select(journal, n) %>%
  arrange(n) # All the ones I have not done manually have < 3000 papers
...
new.metadata %>%
  ...
  select(short.title, n) %>%
  arrange(n) # All the journals I have not done manually have < 3000 papers
...
pubmed_sqlite$journal[substr(pubmed_sqlite$journal, num.char, num.char) == " "] <- substr(
  pubmed_sqlite$journal[substr(pubmed_sqlite$journal, num.char, num.char) == " "], 1,
...
full.summary <- do.call("rbind", lapply(1:length(journals), function(journal.i) {
  ...
  focal <- pubmed_sqlite %>%
    select(journal, discipline, country, date, gender.95) %>%
    filter(journal == journals[journal.i]) %>%
    collect(n = Inf) # get the data on the focal journal
  ...
  author.list <- lapply(1:length(years), function(year.i) {
    ...
    author.list.for.this.year <- lapply(1:length(split.gender.95), function(paper.i) {
      ...
      if (num.authors == 2) {
        author.list.for.this.paper <- c(
          paste(focal.countries[1], "First", focal.genders[1], sep = "_"),
    ...
    author.list.for.this.year <- do.call("rbind", author.list.for.this.year) %>%
      group_by(author.list.for.this.paper) %>%
      summarise(n = sum(value)) %>%
      mutate(year = years[year.i])
    return(author.list.for.this.year)
  })

  author.list <- do.call("rbind", author.list) %>%
    group_by(author.list.for.this.paper, year) %>%
    summarise(n = sum(n)) %>%
    rename(author.type = author.list.for.this.paper)
  ...
  mutate(year = as.numeric(year), country = capitalise.countries(country)) %>%
  arrange(journal, country, year, position, gender)
...
columns <- pubmed_sqlite %>%
  select(forenames, country, gender.95) %>%
  collect(n = Inf) %>%
  as.data.frame()
split.names <- strsplit(columns$forenames, split = "_")
columns <- columns %>% select(-forenames)
split.country <- strsplit(columns$country, split = "_")
columns <- columns %>% select(-country)
split.gender <- strsplit(columns$gender.95, split = "")
rm(columns)
...
data.to.make.country.summary <- data.frame(
  gender = unlist(strsplit(split.gender2, split = "")),
...
country.summary.by.year.and.journal <- data.to.make.country.summary %>%
  group_by(country, gender.95, year, journal, discipline) %>%
  summarise(count = n()) %>%
  as.data.frame()
...
success.rate <- country.summary.by.year.and.journal %>%
  group_by(country) %>%
  summarise(success.rate = sum(count[gender.95 %in% c("M", "F")]) / sum(count[gender.95 %in% c("M", "F", "U")]), n = sum(count[gender.95 %in% c("M", "F", "U")])) %>%
  as.data.frame() %>%
  arrange(success.rate) %>%
  mutate(country = as.character(country))
success.rate$country <- as.character(sapply(success.rate$country, function(x) {
  s <- strsplit(x, " ")[[1]]
