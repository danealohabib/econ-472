...
parse.xml.file <- function(filepath) {
  ...
  rough.data <- capture.output(xmlEventParse(
    file = filepath,
    handlers = NULL,
  ...
  clean.rough.data <- function(rough.data) {
    ...
    cleaned <- do.call("rbind", lapply(cleaned, function(x) {
      ...
      tidy <- function(item, tag) {
        ...
        if (length(focal) == 1) {
          return(gsub(paste(tag, "___", sep = ""), "", focal))
        } else {
          return(NA)
        }
      }
...
parse.many.xml.files <- function(input.dir, output.dir, focal.chunk = NULL, num.chunks = 8, over.write = F) {
  ...
  if (!is.null(focal.chunk)) { # Specify the focal chunk of files
    ...
    if (focal.chunk > num.chunks) {
      return("Check your inputs, they're wrong!")
    }
  ...
  get.journal.info <- function(xml.item) {
    ...
    if (exists("mesh.headings.list")) { # if the list exists and has something in it...
      if (length(mesh.headings.list) > 0) {
        ...
        if (length(mesh.headings) == 0) {
          mesh.headings <- NA
        } # Fill in NA if that got rid of everything
...
get.unique.states <- function(dirs) {
  ...
  states <- melt(with(states, tapply(count, state, sum)))
  names(states) <- c("state", "count")
...
get.unique.cities <- function(dirs, known.states = states$state[!is.na(states$country)]) {
  ...
  cities <- melt(with(cities, tapply(count, city, sum)))
  names(cities) <- c("city", "count")
  ...
  xx <- rbind(
    xx[xx$ISO.3166.1.country.code == "US", ], # Major countries first (this stops it assuming we mean the philadelphia in Jamaica etc)
...
parse.one.address.file <- function(file, matches, matches.with.spaces) {
  ...
  if (nrow(focal) == 0 | !is.character(focal$city) | !is.character(focal$state) | !is.character(focal$country)) {
    return(data.frame(pmid = numeric(0), country = numeric(0)))
  }
...
make.address.datafile <- function(input.dirs, over.write = F, acceptable.countries, state.countries, city.countries, focal.chunk = NULL, num.chunks = 6) {
  ...
  matches <- rbind(
    ...
    as.data.frame(rbind(
      ...
      c("rica", "costa rica"),
      c("papua", "papua new guinea")
    )) %>% rename(city = V1, country = V2)
  ) %>% rename(search.term = city)
